...
移工看護在台灣的照護工作常常是高壓、低支持、跨語言又不容易求助的情境。她們需要一個可以安全記錄照護狀況、整理情緒、與雇主低摩擦溝通的工具，而不是再多一個複雜的管理系統。

本專案的目標使用者是家庭移工看護與雇主。看護端提供手機 Web App，讓她可以記錄排程、生命徵象、日記與 AI 對話；雇主端則退到 LINE 官方帳號，接收必要摘要與提醒。預期影響是降低溝通成本、及早發現高壓訊號，並保護看護的私密內容不被過度揭露。
