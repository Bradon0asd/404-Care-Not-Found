...
介面支援中文／印尼文切換，可在建檔頁或 Tab05 選擇；只有語言偏好會儲存並在重新整理後保留。固定文案與示範內容使用本地語系表，使用者輸入維持原文，詳見[語系維護說明](frontend/src/i18n/README.md)。
...
LINE 登入／推播、語音辨識、使用者內容即時翻譯、AI 即時回覆與付款仍待串接；畫面中的既有 AI 對話與便利貼已讀狀態為示範資料。登入流程尚未提供正式身分驗證與路由保護。
...
└── backend/
    ├── app/                 blueprints、service layer 與 models
    ├── docs/                API 清單與 migration 說明
    └── tests/               pytest
```

- [前端架構與串接說明](frontend/docs/overview.md)
- [後端 API 功能清單](backend/docs/api.md)
- [專案定位與協作規則](CLAUDE.md)

## 後端

Flask REST API，採 App Factory 與 Service Layer，資料庫走 SQLAlchemy + Flask-Migrate。
...
- 完整端點清單、欄位與錯誤碼：[後端 API 功能清單](backend/docs/api.md)
- Swagger UI `GET /api/docs/swagger`、OpenAPI JSON `GET /api/docs/openapi.json`
- Models 與 migration 流程：[Models 與資料庫 Migration](backend/docs/models.md)

正式部署前請設定 `DATABASE_URL`、`LINE_CHANNEL_ACCESS_TOKEN`、`LINE_CHANNEL_SECRET`、`LINE_LOGIN_CHANNEL_ID`、`LINE_LOGIN_CHANNEL_SECRET`，並關閉 debug mode。
