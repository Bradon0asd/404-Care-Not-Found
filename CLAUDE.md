...
給移工看護一個記錄台灣生活與心情的私人空間（樹洞），用遊戲化維持動力；雇主端不做 App，改走 LINE 官方帳號做低頻的「一眼看懂 + 收通知」。
...
- **care log ≠ medical record。** 只做照護生活紀錄，不做醫療診斷、不取代醫療判斷。遇到醫療判斷情境，AI 一律回「建議聯繫家屬／就醫」；生命徵象只呈現「變化」（如「與過去一週相比偏高」），不下醫療判讀，不用紅色標示異常。
