...
def create_note(args):
    note = sticky_note_service.create_note(current_user=g.current_user, **args)
...
def list_notes(args):
    notes = sticky_note_service.list_notes(current_user=g.current_user, **args)
...
def get_note(note_id):
    note = sticky_note_service.get_note(current_user=g.current_user, note_id=note_id)
...
def review_note(note_id):
    note = sticky_note_service.review_note(current_user=g.current_user, note_id=note_id)
    return api_success(StickyNoteSchema().dump(note))
...
def delete_note(note_id):
    sticky_note_service.delete_note(current_user=g.current_user, note_id=note_id)
    return api_success()
