...
**只有路由 stub、還沒做畫面：** Tab02 秘密日記、Tab03 跟我聊聊、Tab04 便利貼牆、Tab05 帳戶管理（`/diary`、`/chat`、`/board`、`/account`，目前都顯示「開發中」佔位頁）
...
| `/diary` `/chat` `/board` `/account` | PlaceholderView | Tab02-05 佔位頁 |
...
src/
  components/
    ...
    tab02-diary/ tab03-chat/ tab04-board/ tab05-account/  預留給後續 Tab，目前是空的
  views/        每個路由對應一個 view，負責組裝 components + 串 store
  stores/       Pinia，一個 Tab/流程一個 store（onboarding.ts、schedule.ts）
  router/
