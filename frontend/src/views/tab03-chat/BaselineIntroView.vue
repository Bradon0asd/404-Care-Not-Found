...
import WordCloud from '@/components/tab03-chat/WordCloud.vue'
  ...
  <PageContainer>
    ...
    <div class="flex-1 space-y-4 px-4 pb-4">
      <div class="space-y-3 rounded-xl bg-ink-200 p-4 text-center">
        <p class="text-sm text-ink-700">{{ $t('一起聊聊你最近的狀態吧') }}</p>
        <WordCloud />
        <div class="mx-auto max-w-[220px]">
          <BaseButton variant="primary" @click="router.push('/chat/baseline/questions')">{{
            $t('完成一次性小任務')
          }}</BaseButton>
        </div>
      </div>

      <ExpandableSection :title="$t('說明與免責聲明')">
        <p>
          {{
            $t(
