    ...
    >
      <div class="w-full max-w-xs rounded-2xl bg-white p-5 shadow-lg">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-sm font-bold text-ink-950">動態邀請碼</h2>
          <button type="button" class="text-ink-600" @click="$emit('close')" aria-label="關閉">
        ...
        <div class="mb-4 flex items-center gap-2 rounded-full border border-ink-400 px-4 py-3">
          <svg class="h-4 w-4 shrink-0 text-ink-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M10 13a5 5 0 0 0 7 0l2-2a5 5 0 0 0-7-7l-1 1M14 11a5 5 0 0 0-7 0l-2 2a5 5 0 0 0 7 7l1-1" />
          </svg>
          <span class="truncate text-xs text-ink-700">{{ inviteLink }}</span>
        </div>

        <BaseButton variant="primary" @click="copyLink">{{ copied ? '已複製' : '複製邀請碼' }}</BaseButton>
      </div>
    </div>
  </Teleport>
</template>
